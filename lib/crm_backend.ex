defmodule CrmBackend do
  @moduledoc """
  CrmBackend keeps the contexts that define your domain
  and business logic.
  ...
  def get_columns(module) do
    mod = Module.concat(["CrmBackend", "Settings", module])
  ...
  def s_to_map(struct, exclusion \\ []) do
    ...
    d = Enum.filter(a, fn x -> elem(x, 1) end)
    ...
    j =
      for item <- i do
        if a[elem(item, 0)] != nil do
          {elem(item, 0), Poison.decode!(a[elem(item, 0)])}
        else
          []
        end
      end
